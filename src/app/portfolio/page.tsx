...
// ──────────────────────────────────────────────────────────────────────────
//  BEFORE / AFTER TRANSFORMATIONS  —  the only place to edit to add a pair.
//
//  TO ADD A NEW TRANSFORMATION:
//    1. Drop two files in /public/images named:
//         {Project-Name}-before.{ext}  +  {Project-Name}-after.{ext}
//    2. Add ONE line to the array below with that {Project-Name} as `stem`.
//
//  The title is auto-derived from `stem` (hyphens → spaces). Both halves use
//  `ext` unless they differ — then set `before` / `after` per half (e.g. the
//  Hallway pair below is .jpg / .jpeg). Spaces & "&" in names are fine.
// ──────────────────────────────────────────────────────────────────────────
const TRANSFORMATION_PAIRS: { stem: string; ext?: string; before?: string; after?: string }[] = [
  { stem: "Commercial-Office-Decorating", ext: "png" },
  { stem: "Complete-Exterior-Repaint", ext: "jpg" },
  { stem: "Consumer-Unit-Upgrade", ext: "jpg" },
  { stem: "Feature-Wall-Design", ext: "png" },
  { stem: "Full-Electrical-Rewiring", ext: "png" },
  { stem: "Kitchen-Repaint-&-Refresh", ext: "png" },
  { stem: "Smart-Lighting-Installation", ext: "png" },
  { stem: "Hallway & Staircase-Renovation", before: "jpg", after: "jpeg" },
  { stem: "Interior-living-room-Transformation", before: "jpeg", after: "jpg" },
];

const TRANSFORMATIONS = TRANSFORMATION_PAIRS.map((p) => {
  const beforeExt = p.before ?? p.ext;
  const afterExt = p.after ?? p.ext;
  return {
    title: p.stem.replace(/-/g, " "),
    before: `/images/${p.stem}-before.${beforeExt}`,
    after: `/images/${p.stem}-after.${afterExt}`,
  };
});

// Pre-filled WhatsApp deep-link for a given project's "Request Similar" CTA.
const waFor = (title: string) =>
  `https://wa.me/447922909982?text=${encodeURIComponent(`Hi Property Care, I'd like a transformation similar to "${title}". Please send me a free quote.`)}`;
  ...
  return (
    <>
      ...
      <main id="top">
        ...
        {/* ══════════════════════════════════════
            BEFORE & AFTER  (data-driven — one slider per TRANSFORMATIONS entry)
        ══════════════════════════════════════ */}
        <section style={{ padding: "120px 52px", background: "#FDFCFB" }} id="before-after">
          <div style={{ maxWidth: 1400, margin: "0 auto" }}>
            <div className="rv" style={{ textAlign: "center", marginBottom: 20 }}>
              <OrangeTab center>Real Results</OrangeTab>
              <h2 className="rv rv-d1" style={{
            ...
            <div className="rv rv-d2" style={{ maxWidth: 720, margin: "0 auto 64px", textAlign: "center" }}>
              <p style={{ fontSize: 16, color: "#3D3A37", lineHeight: 1.7, fontWeight: 400 }}>
                Drag the handle on any project to slide between before and after. Every transformation is documented, dated and backed by our written workmanship guarantee.
              </p>
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48 }} className="grid-2">
              {TRANSFORMATIONS.map((project, i) => (
                <div key={project.title} className={`ba-card rv rv-d${(i % 3) + 1}`}>
                  <BeforeAfterSlider
                  ...
                  <div style={{ paddingTop: 22 }}>
                    <h3 style={{ fontSize: 22, fontWeight: 800, color: "#0A0908", letterSpacing: "-0.01em", marginBottom: 16 }}>
                      {project.title}
                    </h3>
                    <a href={waFor(project.title)} className="btn-orange">
                      <span>
            ...
            <p className="rv" style={{ textAlign: "center", marginTop: 48, fontSize: 13, color: "#9E9488", letterSpacing: "0.04em", fontWeight: 500 }}>
              Prefer to talk? Use the WhatsApp, text and call buttons on screen any time.
            </p>
          </div>
        </section>
